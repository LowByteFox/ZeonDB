...
fn alloc_buff(handle: [*c]uv.uv_handle_t, suggest: usize, buf: [*c]uv.uv_buf_t) callconv(.C) void {
    ...
    zigbuf.len = suggest;
    if (data.allocated > 0) {
        const old_size = data.allocated;
        data.allocated += suggest;
        data.inbuff = @ptrCast(std.c.realloc(@ptrCast(data.inbuff), data.allocated));
        zigbuf.base = data.inbuff + old_size;
    } else {
        data.inbuff = @ptrCast(std.c.malloc(suggest));
        data.allocated = suggest;
...
fn on_read(stream: [*c]uv.uv_stream_t, nread: isize, buf: [*c]const uv.uv_buf_t) callconv(.C) void {
    ...
    data.allocated -= @intCast(zigbuf.len);
    if (nread > 0) {
        data.allocated += @intCast(nread);
        data.inbuff = @ptrCast(std.c.realloc(@ptrCast(data.inbuff), data.allocated));
        if (data.allocated % 1024 == 0) {
            data.frame = networking.frame.ZeonFrame {
                .status = .Ok,
                .target_length = 0,
                .fixed_buffer = undefined
            };
